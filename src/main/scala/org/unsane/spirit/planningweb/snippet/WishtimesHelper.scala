...
trait WishtimesHelper {
  ...
  val persistenceRoom:IPersistence = PersistenceFactory
                                       .createPersistence(TransformFactory
                                       .createTransformRoom(persLayer))
  ...
  val rooms = persistenceRoom.read.asInstanceOf[List[Room]]
  ...
  // to get all selected rooms of a dozent
  def getSelectedRooms() = {
    worktimeOfDozent match {
      case List() => List()
      case _ => worktimeOfDozent.head.rooms
    }
  }

  // to check if a room is selected
  def isSelectedRoom(room: Room) = {
    val rooms = getSelectedRooms
    rooms.contains(room)
  }
}